...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'GreyNoise Community');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  switch (name) {
    case 'ip_context':
      return ipContext(apiKey, reqStr(args, 'ip', '"8.8.8.8"').trim());
    default:
...
async function ipContext(apiKey: string, ip: string) {
  ...
  const res = await pwFetch(`${BASE_URL}/${encodeURIComponent(ip)}`, {
